...
use super::old_reader::{MalType, ParseError};
use rustyline::{error::ReadlineError, DefaultEditor};

use super::old_env::Env;

use self::{
    super::old_core::{add_premade_lisp_fn_to, create_core_namespace},
    super::old_reader::SpecialKeyword,
};
...
pub fn new_eval_error<T>(msg: String) -> Result<T, ReplError> {
    Err(ReplError::Eval(msg))
}

/// Read in a string and parse it into an AST expression
pub fn read(line: String) -> MalResult {
    let expr = *super::old_reader::read_str(&line).map_err(ReplError::Parse)?;
    Ok(expr)
}
...
pub fn eval(ast: MalType, env: Env) -> MalResult {
    let mut current_ast = ast;
...
fn print(value: MalType) {
    println!("{}", super::old_printer::pr_str(value, true))
}

/// Runs the read, evaluate, and print functions in that order
pub fn rep(line: String, env: &Env) -> Result<(), ReplError> {
    let ast = read(line)?;
...
#[cfg(test)]
mod tests {
    use crate::old_core::stringify_args;
    ...
    pub fn simulate_print(args: Vec<MalType>) -> MalResult {
        let string = stringify_args(args, true, Some(" "));
        unsafe { OUTPUT.push(string) };
        Ok(MalType::Nil(()))
    }
    pub fn simulate_println(args: Vec<MalType>) -> MalResult {
        let string = stringify_args(args, false, Some(" "));
        ...
        for (number, line) in file.lines().enumerate().map(|(n, l)| (n + 1, l)) {
            if line.is_empty() || line.starts_with(";;") || line.starts_with(";>>>") {
                ...
                if let Ok(ref success) = result {
                    assert_eq!(
                        crate::old_printer::pr_str(success.clone(), true),
                        output,
                ...
                result = eval(
                    *crate::old_reader::read_str(line).expect("Invalid Input"),
                    test_env.clone(),
                );
            }
            if print_line {
